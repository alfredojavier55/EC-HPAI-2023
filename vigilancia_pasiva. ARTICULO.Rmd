...
# Cotejando wahis, sizse 
```{r}
v3$definitivo2[v3$orden == 237] <- "notifi"
...
l <- read_xlsx("Casos IA comparacion base y WAHIS.xlsx")
...
lutuk_kernel <- 
  # ggmap(lu2) +
  stat_density_2d(data = l, aes(x = x, y = y, fill = ..level..),
                  geom = "polygon", alpha = 0.4) +  # Heatmap using density
  scale_fill_viridis_c() +  # Color scale for heatmap
  geom_point(data = filtered_data, aes(x = x, y = y), size = 0.1, 
             color = "red") +  # Points on top of heatmap
  facet_wrap(
    ~ month_mod, 
    nrow = 2, ncol = 3,
    labeller = as_labeller(function(x) {
      labels <- c("1" = "Dec 2023", "2" = "Jan 2024", "3" = "Feb 2024", 
